    ...
    <el-tabs v-if="config" v-model="tab" tab-position="left" class="designer-body">
      ...
      <el-tab-pane label="权限" name="acl">
        <el-form label-width="110px" class="form-panel">
          ...
          <el-form-item label="角色 ID"><el-input v-model="roleIds" placeholder="多个 ID 使用逗号分隔" /></el-form-item>
          <el-form-item label="用户 ID"><el-input v-model="userIds" placeholder="多个 ID 使用逗号分隔" /></el-form-item>
        </el-form>
      </el-tab-pane>
      <el-tab-pane label="查询控件" name="controls">
        ...
        <el-table :data="config.controls" border row-key="controlKey">
          <el-table-column type="expand">
            <template #default="scope">
              <div class="expand-panel">
                <el-form inline label-width="90px">
                  <el-form-item label="目标区域"><el-select v-model="scope.row.targetComponentKeys" multiple style="width:360px"><el-option v-for="item in config.components" :key="item.componentKey" :label="item.componentName" :value="item.componentKey" /></el-select></el-form-item>
                  <el-form-item v-if="scope.row.optionSource === 'SQL'" label="选项数据源"><el-select v-model="scope.row.optionDatasourceId" style="width:240px"><el-option v-for="item in sources" :key="item.id" :label="item.datasourceName" :value="item.id" /></el-select></el-form-item>
                </el-form>
                <el-form-item v-if="scope.row.optionSource === 'SQL'" label="只读 SQL" label-width="90px"><el-input v-model="scope.row.optionSql" type="textarea" :rows="3" placeholder="SELECT value, label FROM ..." /></el-form-item>
          ...
          <el-table-column label="类型" width="155"><template #default="{ row }"><el-select v-model="row.controlType"><el-option v-for="type in controlTypes" :key="type" :label="type" :value="type" /></el-select></template></el-table-column>
          <el-table-column label="选项来源" width="115"><template #default="{ row }"><el-select v-model="row.optionSource"><el-option label="无" value="NONE"/><el-option label="静态" value="STATIC"/><el-option label="SQL" value="SQL"/></el-select></template></el-table-column>
      ...
      <el-tab-pane label="参数映射" name="mappings">
        ...
        <el-table :data="defaultMappings" border>
          ...
          <el-table-column prop="mysqlDataType" label="MySQL 类型" width="130" />
          <el-table-column label="来源" width="145"><template #default="{ row }"><el-select v-model="row.sourceType" :disabled="isReservedParameter(row.parameterName)"><el-option v-for="type in sourceTypes" :key="type" :label="type" :value="type" /></el-select></template></el-table-column>
...
import { diffReportVersion, getReportConfiguration, listReportVersions, rollbackReportVersion, saveReportConfiguration, validateReportConfiguration, type ParameterMapping, type ReportConfiguration, type ReportComponent, type ReportControl, type ReportRoute, type VersionSummary } from '@/api/bi/report'
...
const roleIds = ref(''); const userIds = ref('')
...
const controlTypes = ['TEXT', 'SINGLE_SELECT', 'MULTI_SELECT', 'DATE', 'DATE_RANGE', 'NUMBER', 'NUMBER_RANGE']
const sourceTypes = ['SYSTEM', 'REGION', 'COMPONENT', 'CONTROL', 'DRILL', 'CONSTANT', 'NULL']
...
const parseIds = (value: string) => value.split(',').map(v => v.trim()).filter(v => /^\d+$/.test(v))

async function load() {
  loading.value = true
  try {
    const [configuration, datasourcePage] = await Promise.all([getReportConfiguration(reportId), listDatasources({ page: 1, pageSize: 100, status: 'ENABLED' })])
    ...
    roleIds.value = config.value.acl.roleIds.join(','); userIds.value = config.value.acl.userIds.join(',')
    await Promise.all([sourceChanged(config.value.baseInfo.defaultDatasourceId, false), loadVersions()])
...
async function save() {
  ...
  config.value.acl.roleIds = parseIds(roleIds.value); config.value.acl.userIds = parseIds(userIds.value)
  saving.value = true
  try {
